...
class MLService {
  ...
  async recordUserAction(deviceId, action, appliances = null) {
    const engine = this.getCurrentEngine();
    if (!engine) {
      console.warn('⚠️ ML Engine not available for recording user action');
    ...
    try {
      ...
      if (appliances && Array.isArray(appliances)) {
        deviceList = appliances;
      } else if (this.currentAppliances && Array.isArray(this.currentAppliances)) {
        deviceList = this.currentAppliances;
      }
      ...
      if (!device) {
        console.warn(`⚠️ Device ${deviceId} not found for recording action`);
      ...
      const userAction = {
        ...
        action, // 'turn_on', 'turn_off', etc.
        hour: currentHour,
        dayOfWeek,
        isWeekend,
        context: {
          room: device.room,
          power: device.normal_usage || 0,
          manualAction: true
        }
      };
      ...
      engine.trainingData.userActions.push(userAction);

      // Keep data manageable (last 1000 actions)
      if (engine.trainingData.userActions.length > 1000) {
        engine.trainingData.userActions = engine.trainingData.userActions.slice(-500);
      }

      console.log(`📝 Recorded user action: ${action} for ${device.name} at ${currentHour}:00`);

      // Save to storage
      await engine.saveTrainingData();

      // Trigger data collection to capture current state (only if we have devices)
      if (deviceList.length > 0) {
        await this.collectData(deviceList);
      }

      return { 
        success: true, 
        action: userAction,
        totalActions: engine.trainingData.userActions.length 
      };
  ...
  async fastForwardSimulation(days = 1, onProgress = null) {
    ...
    console.log(`🚀 ML Fast-forwarding ${days} day(s) with full day patterns...`);
