...
const HomeTab = () => {
  ...
  const saveAppliance = async () => {
    ...
    const usage = parseInt(newAppliance.normal_usage, 10);
    ...
    try {
      const { error } = await supabase
        ...
        .insert([{
          ...
          room: newAppliance.room.trim(),
          normal_usage: usage,
          average_hours_per_day: parseInt(newAppliance.average_hours_per_day, 10) || 8,
          status: 'off',
        }]);
      ...
      >
        <View style={styles.modalOverlay}>
          <View style={styles.modalContainer}>
            ...
            {/* -----  ADD-DEVICE MODAL (same as ControlsTab)  ----- */}
      <Modal
      ...
      >
        ...
        >
          <SafeAreaView style={{ flex: 1, backgroundColor: '#fff' }}>
            {/* Header */}
            <View style={{ flexDirection: 'row', alignItems: 'center', justifyContent: 'space-between', padding: 16, borderBottomWidth: 1, borderColor: '#e5e7eb' }}>
              <TouchableOpacity onPress={closeAddModal}>
                <Text style={{ color: '#6b7280', fontSize: 16 }}>Cancel</Text>
              </TouchableOpacity>
              <Text style={{ fontSize: 18, fontWeight: '600' }}>Add New Device</Text>
              <TouchableOpacity onPress={saveAppliance}>
                <Text style={{ color: '#10b981', fontSize: 16 }}>Save</Text>
              </TouchableOpacity>
            </View>
            ...
            <ScrollView contentContainerStyle={{ padding: 16 }}>
              ...
              <View style={{ marginBottom: 16 }}>
                <Text style={{ marginBottom: 6, fontWeight: '500' }}>Device Name *</Text>
                <TextInput
                  style={{ borderWidth: 1, borderColor: '#d1d5db', borderRadius: 8, padding: 12, fontSize: 16 }}
                  placeholder="Enter device name"
                  value={newAppliance.name}
                  onChangeText={t => setNewAppliance(p => ({ ...p, name: t }))}
                />
              </View>

              {/* Type */}
              <View style={{ marginBottom: 16 }}>
                <Text style={{ marginBottom: 6, fontWeight: '500' }}>Device Type *</Text>
                <ScrollView horizontal showsHorizontalScrollIndicator={false}>
                  <View style={{ flexDirection: 'row' }}>
                    {[
                      ...
                      <TouchableOpacity
                        key={t.key}
                        onPress={() => setNewAppliance(p => ({ ...p, type: t.key }))}
                        style={{
                          alignItems: 'center',
                          marginRight: 12,
                          padding: 8,
                          borderRadius: 8,
                          backgroundColor: newAppliance.type === t.key ? '#10b981' : '#f3f4f6',
                        }}>
                        <MaterialIcons name={t.icon} size={24} color={newAppliance.type === t.key ? '#fff' : '#6b7280'} />
                        <Text style={{ marginTop: 4, fontSize: 12, color: newAppliance.type === t.key ? '#fff' : '#6b7280' }}>{t.label}</Text>
                      </TouchableOpacity>
                    ))}
                  </View>
                </ScrollView>
              ...
              {/* Room */}
              <View style={{ marginBottom: 16 }}>
                <Text style={{ marginBottom: 6, fontWeight: '500' }}>Room *</Text>
                <TextInput
                  style={{ borderWidth: 1, borderColor: '#d1d5db', borderRadius: 8, padding: 12, fontSize: 16 }}
                  placeholder="e.g. Living Room"
                  value={newAppliance.room}
                  onChangeText={t => setNewAppliance(p => ({ ...p, room: t }))}
                />
              </View>

              {/* Power Usage */}
              <View style={{ marginBottom: 16 }}>
                <Text style={{ marginBottom: 6, fontWeight: '500' }}>Power Usage (Watts) *</Text>
                <TextInput
                  style={{ borderWidth: 1, borderColor: '#d1d5db', borderRadius: 8, padding: 12, fontSize: 16 }}
                  placeholder="Enter wattage"
                  keyboardType="numeric"
                  value={newAppliance.normal_usage}
                  onChangeText={t => setNewAppliance(p => ({ ...p, normal_usage: t.replace(/[^0-9]/g, '') }))}
                />
              </View>

              {/* Daily Hours */}
              <View style={{ marginBottom: 16 }}>
                <Text style={{ marginBottom: 6, fontWeight: '500' }}>Average Daily Usage (Hours)</Text>
                <TextInput
                  style={{ borderWidth: 1, borderColor: '#d1d5db', borderRadius: 8, padding: 12, fontSize: 16 }}
                  placeholder="e.g. 8"
                  keyboardType="numeric"
                  value={newAppliance.average_hours_per_day}
                  onChangeText={t => setNewAppliance(p => ({ ...p, average_hours_per_day: t.replace(/[^0-9]/g, '') }))}
                />
              </View>
            </ScrollView>
          </SafeAreaView>
