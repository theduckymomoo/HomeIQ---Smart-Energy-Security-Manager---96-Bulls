...
const AREA_CODE_STORAGE_KEY = '@loadshedding_area_code';
...
const LoadsheddingTab = ({ loadshedding, onRefresh }) => {
  ...
  const [inputAreaCode, setInputAreaCode] = useState('');
  ...
  const loadAreaCode = async () => {
    try {
      const stored = await AsyncStorage.getItem(AREA_CODE_STORAGE_KEY);
      if (stored) {
        setAreaCode(stored);
        setInputAreaCode(stored);
      }
    } catch (error) {
  ...
  const saveAreaCode = async (code) => {
    try {
      await AsyncStorage.setItem(AREA_CODE_STORAGE_KEY, code);
      setAreaCode(code);
    } catch (error) {
  ...
  const handleAreaCodeChange = async () => {
    if (!inputAreaCode.trim()) {
      Alert.alert('Invalid Input', 'Please enter a valid area code');
      return;
    }
    ...
    try {
      // Save the area code
      await saveAreaCode(inputAreaCode.trim());
      
      // Trigger a refresh with the new area code
      if (onRefresh) {
        await onRefresh(inputAreaCode.trim());
      }
      ...
      Alert.alert(
        'Area Updated',
        `Successfully updated your area to ${inputAreaCode.trim()}. Loadshedding information has been refreshed.`,
        [{ text: 'OK' }]
      );
    } catch (error) {
  ...
  const handleRefresh = async () => {
    setRefreshing(true);
    await Promise.all([onRefresh(areaCode), fetchAppliances()]);
    setRefreshing(false);
  };
  ...
  return (
    ...
    >
      {/* Current Stage Card */}
      <TouchableOpacity
        style={[styles.stageCard, { borderColor: stageInfo.color }]}
      ...
      <View style={styles.automationCard}>
        <View style={styles.automationHeader}>
          <View style={styles.automationLeft}>
            <MaterialIcons 
              name={automationEnabled ? "auto-awesome" : "settings"} 
              size={24} 
      ...
      {/* Area Info - NOW TAPPABLE */}
      {loadshedding.area && (
        <TouchableOpacity
          style={styles.infoCard}
          onPress={() => {
            Vibration.vibrate(50);
            setInputAreaCode(areaCode);
        ...
        >
          ...
          <Text style={styles.infoText}>
            {areaCode || loadshedding.area}
          </Text>
          <Text style={[styles.infoText, { fontSize: 12, marginTop: 8, color: '#6b7280' }]}>
            Tap to change your area code
          </Text>
        </TouchableOpacity>
      )}
        ...
        <View style={styles.modalOverlay}>
          <View style={styles.areaModalContainer}>
            <View style={styles.modalHeader}>
              <Text style={styles.modalTitle}>Change Area Code</Text>
            ...
            <View style={styles.areaModalContent}>
              <Text style={styles.areaModalLabel}>Enter your area code:</Text>
              <TextInput
                style={styles.areaInput}
                value={inputAreaCode}
                onChangeText={setInputAreaCode}
                placeholder="e.g., gauteng-1-johannesburg"
                placeholderTextColor="#6b7280"
                autoCapitalize="none"
                autoCorrect={false}
              />
              
              <Text style={styles.areaModalHint}>
                Enter your municipality or area code to get accurate loadshedding schedules.
              </Text>

              <TouchableOpacity
                style={[styles.areaModalButton, isLoadingArea && styles.areaModalButtonDisabled]}
                onPress={handleAreaCodeChange}
              ...
              >
                {isLoadingArea ? (
                  ...
                  <>
                    ...
                    <Text style={styles.areaModalButtonText}>Update Area</Text>
                  </>
                )}
              </TouchableOpacity>
            </View>
      ...
      <Modal
        ...
        animationType="slide"
        transparent={false}
        onRequestClose={() => setShowAutomationModal(false)}
      >
        <View style={styles.modalContainer}>
          ...
          <ScrollView style={styles.modalContent} contentContainerStyle={styles.contentContainer}>
            <View style={styles.settingSection}>
              <Text style={styles.settingTitle}>Auto-Off Rules</Text>

              <View style={styles.settingItem}>
                <View style={styles.settingLeft}>
                  <MaterialIcons name="flash-on" size={24} color="#f97316" />
              ...
              <View style={styles.settingItem}>
                <View style={styles.settingLeft}>
                  <MaterialIcons name="power" size={24} color="#6b7280" />
      ...
      <Modal
        ...
        animationType="slide"
        transparent={false}
        onRequestClose={() => setShowStageDetailsModal(false)}
      >
        <View style={styles.modalContainer}>
          <View style={styles.modalHeader}>
            <View />
          ...
          <ScrollView style={styles.modalContent} contentContainerStyle={styles.contentContainer}>
            ...
            <View style={[styles.stageDetailCard, { borderColor: stageInfo.color }]}>
              <View style={styles.stageDetailHeader}>
                <MaterialIcons name={stageInfo.icon} size={48} color={stageInfo.color} />
            ...
            <View style={styles.comparisonSection}>
              ...
              <Text style={styles.comparisonSubtitle}>Tap any stage to see details</Text>
