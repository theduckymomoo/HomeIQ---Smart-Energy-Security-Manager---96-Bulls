...
export default function AnalysisTab({ appliances: propAppliances = [], stats = {} }) {
  ...
  // FIXED: Only use prop appliances - no local fetching
  const appliances = propAppliances || [];
  ...
  const [simulatedAppliances, setSimulatedAppliances] = useState([]);
  ...
  // FIXED: Update ML only when appliances actually change
  useEffect(() => {
    if (appliances.length > 0) {
      console.log('🔄 Appliances updated, refreshing ML:', appliances.length);
      mlService.updateAppliances(appliances);
      refreshMLInsights();
    }
  }, [appliances.length]); // Only trigger when length changes

  // Force refresh when simulation modal closes
  useEffect(() => {
    if (!showSimulationModal && mlService.hasCurrentUser()) {
      console.log('🔄 Simulation modal closed, refreshing analytics...');
      setTimeout(() => {
        refreshMLInsights();
      }, 500);
    }
    ...
    try {
      ...
      if (insights.ready) {
        ...
        if (predsResult.success) {
          ...
          console.log('🔮 Predictions:', predsResult.predictions?.length || 0);
        } else {
          setPredictions([]);
          console.log('⚠️ No predictions available:', predsResult.error);
        }

        const recs = mlService.getRecommendations(appliances);
        setRecommendations(recs || []);
        console.log('💡 Recommendations:', recs?.length || 0);

        const anom = mlService.detectAnomalies(appliances);
        setAnomalies(anom || { hasAnomaly: false, anomalies: [] });
        console.log('⚠️ Anomalies:', anom?.hasAnomaly ? anom.anomalies.length : 0);

        const forecast = mlService.getEnergyForecast(appliances, 12);
        setEnergyForecast(forecast || { success: false, forecast: [] });
        console.log('📈 Energy Forecast:', forecast?.success ? 'Available' : 'Not available');
      } else {
        setPredictions([]);
        setRecommendations([]);
        setAnomalies({ hasAnomaly: false, anomalies: [] });
        setEnergyForecast(null);
        console.log('⏳ ML not ready, progress:', progress.progress + '%');
      }
    } catch (error) {
  ...
  const getTypicalUsagePattern = (applianceType) => {
    const patterns = {
      refrigerator: Array(24).fill(0.3),
    ...
    switch (selectedTimeFrame) {
      case 'day':
        ...
        return {
          ...
          datasets: [{
            data: [
              Math.round(hourlyData[6] / 100),
              Math.round(hourlyData[9] / 100),
              Math.round(hourlyData[12] / 100),
              Math.round(hourlyData[15] / 100),
              Math.round(hourlyData[18] / 100),
              Math.round(hourlyData[21] / 100),
              Math.round(hourlyData[0] / 100),
            ],
          }],
        };
        ...
        return {
          ...
          datasets: [{
            data: [
              ...
              Math.round(dailyAverage * 0.95),
              Math.round(dailyAverage),
              Math.round(dailyAverage * 1.05),
      ...
      case 'year':
        return {
          ...
          datasets: [{
            data: [
              Math.round(analytics.totalEnergy * 1.1),
  ...
  // FIXED: onRefresh doesn't fetch - parent handles that
  const onRefresh = useCallback(async () => {
    setRefreshing(true);
    Vibration.vibrate(50);
    console.log('🔄 Refreshing ML analytics...');
    ...
    setTimeout(() => {
      setRefreshing(false);
      Alert.alert('✅ Data Updated', 'Your analytics have been refreshed with the latest ML predictions.');
    }, 1000);
  }, [appliances]);
  ...
  const handleSimulationUpdate = async (result) => {
    console.log('📊 Simulation completed, updating analytics...', result);
    
    try {
      setSimulatedAppliances(appliances);
      await refreshMLInsights();

      setTimeout(async () => {
        console.log('🔄 Secondary refresh after simulation...');
        await refreshMLInsights();
      }, 1500);
    } catch (error) {
      console.error('Error updating after simulation:', error);
    }
  };

  const renderMLStatusCard = () => (
    <Animated.View style={[styles.mlStatusCard, { opacity: fadeAnim, transform: [{ translateY: slideAnim }] }]}>
        ...
        <View style={styles.mlStatusContent}>
          <View style={styles.progressBarContainer}>
            <View style={styles.progressBarBackground}>
              <View style={[styles.progressBarFill, { width: `${trainingProgress.progress}%` }]} />
            </View>
          </View>
        ...
        <View style={styles.mlStatusContent}>
          <View style={styles.mlMetrics}>
            <View style={styles.mlMetric}>
              <Text style={styles.mlMetricValue}>{(mlInsights.accuracy * 100).toFixed(1)}%</Text>
            ...
            <View style={styles.mlMetric}>
              <Text style={styles.mlMetricValue}>{predictions.length}</Text>
  ...
  const renderAnomalyAlerts = () => {
    ...
    return (
      <Animated.View style={[styles.anomalyContainer, { opacity: fadeAnim, transform: [{ translateY: slideAnim }] }]}>
        ...
        {anomalies.anomalies.map((anomaly, index) => (
          <View key={index} style={styles.anomalyCard}>
            ...
            <View style={styles.anomalyDetails}>
              <Text style={styles.anomalyDetailText}>Current: {anomaly.currentPower}W</Text>
              <Text style={styles.anomalyDetailText}>Expected: {anomaly.expectedPower}W</Text>
            </View>
          </View>
        ))}
      </Animated.View>
      ...
      >
        <Animated.View style={[styles.section, { opacity: fadeAnim, transform: [{ translateY: slideAnim }] }]}>
          <View style={styles.sectionHeader}>
            ...
            <Text style={styles.expandIcon}>{expandedSection === 'predictions' ? '▼' : '▶'}</Text>
          </View>
              ...
              {predictions.slice(0, 5).map((pred, index) => (
                <View key={`${pred.deviceId}-${index}`} style={styles.predictionCard}>
                  <View style={styles.predictionHeader}>
                    ...
                    <View style={[styles.predictionBadge, pred.prediction.willBeActive ? styles.predictionActive : styles.predictionInactive]}>
                      <Text style={styles.predictionBadgeText}>{pred.prediction.probability}%</Text>
                    </View>
                  </View>
      ...
      >
        <Animated.View style={[styles.section, { opacity: fadeAnim, transform: [{ translateY: slideAnim }] }]}>
          <View style={styles.sectionHeader}>
            ...
            <Text style={styles.expandIcon}>{expandedSection === 'recommendations' ? '▼' : '▶'}</Text>
          </View>
              ...
              {recommendations.map((rec, index) => (
                ...
                >
                  ...
                  {rec.potentialSavings && (
                    <Text style={styles.savingsText}>💰 Save R{rec.potentialSavings}/month</Text>
                  )}
                  {rec.confidence && (
      ...
      >
        <Animated.View style={[styles.section, { opacity: fadeAnim, transform: [{ translateY: slideAnim }] }]}>
          <View style={styles.sectionHeader}>
            ...
            <Text style={styles.expandIcon}>{expandedSection === 'forecast' ? '▼' : '▶'}</Text>
          </View>
            ...
            <Animated.View style={[styles.sectionContent, { opacity: fadeAnim, transform: [{ translateY: slideAnim }] }]}>
              ...
              <View style={styles.forecastSummary}>
                <View style={styles.forecastStat}>
                  <Text style={styles.forecastStatValue}>{energyForecast.totalExpectedEnergy}kWh</Text>
                  <Text style={styles.forecastStatLabel}>Total Energy</Text>
                </View>
                <View style={styles.forecastStat}>
                  <Text style={styles.forecastStatValue}>R{energyForecast.totalExpectedCost}</Text>
  ...
  const renderChart = () => {
    ...
    if (selectedChart === 'energy') {
      ...
      const costData = {
        ...data,
        datasets: [{
          data: data.datasets[0].data.map(value => Math.round(value * 2.5))
        }]
      };
        ...
        return (
          <View>
            ...
            <Text style={styles.modalDescription}>
              Detailed predictive analytics based on your current ACTIVE device usage patterns.
            </Text>
          </View>
        );
    }
  ...
  // FIXED: Better empty state without loading spinner
  if (!appliances || appliances.length === 0) {
    return (
  ...
  return (
    <SafeAreaView style={styles.container}>
      ...
      >
        ...
        {/* Efficiency Card */}
        <Animated.View style={[styles.efficiencyCard, { opacity: fadeAnim, transform: [{ translateY: slideAnim }] }]}>
