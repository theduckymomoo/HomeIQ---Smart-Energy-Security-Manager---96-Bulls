...
export default function ControlsTab() {
  ...
  const updateMLRecommendations = async () => {
    ...
    try {
      ...
      setMLRecommendations(recs.slice(0, 3));
    } catch (error) {
        ...
        <View style={styles.mlBannerHeader}>
          <View style={styles.mlBannerLeft}>
            ...
            <View>
              ...
              <Text style={styles.mlBannerSubtitle}>{mlRecommendations.length} recommendation{mlRecommendations.length > 1 ? 's' : ''}</Text>
            </View>
          </View>
          <MaterialIcons name={showMLInsights ? "expand-less" : "expand-more"} size={24} color="#8b5cf6" />
        </View>
        {!showMLInsights && (
          <Text style={styles.mlBannerPreview}>{topRec.suggestion}</Text>
        )}
        {showMLInsights && (
            ...
            {mlRecommendations.map((rec, index) => (
              <View key={index} style={styles.mlInsightCard}>
                <View style={styles.mlInsightHeader}>
                  <View style={[styles.mlPriorityDot, { backgroundColor: rec.priority === 'high' ? '#ef4444' : rec.priority === 'medium' ? '#f59e0b' : '#10b981' }]} />
                  <Text style={styles.mlInsightType}>{rec.type.replace('_', ' ')}</Text>
                </View>
                <Text style={styles.mlInsightText}>{rec.suggestion}</Text>
                {rec.potentialSavings && (
                  <Text style={styles.mlInsightSavings}>💰 Save R{rec.potentialSavings}/month</Text>
                )}
              </View>
            ))}
          </View>
  ...
  const renderDeviceCard = ({ item }) => {
    ...
    try {
            ...
            <View style={styles.deviceListContent}>
              ...
              <View style={styles.deviceListRight}>
                ...
                {isToggling ? (
                  ...
                  <View style={[styles.statusIndicator, item.status === 'on' ? styles.statusOn : styles.statusOff]}>
                    <Text style={styles.statusText}>{item.status === 'on' ? 'ON' : 'OFF'}</Text>
                  </View>
                )}
                <TouchableOpacity
                ...
                >
                  <MaterialIcons name="more-vert" size={24} color="#a1a1aa" />
                </TouchableOpacity>
              </View>
            </View>
      ...
      return (
        <TouchableOpacity
          style={[styles.deviceCard, item.status === 'offline' && styles.deviceOffline]}
          onPress={() => toggleAppliance(item.id, item.status)}
          activeOpacity={0.7}
          disabled={isToggling}
        >
          ...
          {hasPrediction && (
            <View style={styles.mlPredictionChip}>
              ...
              <Text style={styles.mlPredictionChipText}>{prediction.prediction.probability}%</Text>
            </View>
          )}
          ...
          <View style={styles.deviceCardFooter}>
            ...
            {isToggling ? (
              ...
              <TouchableOpacity
                style={[styles.statusIndicator, item.status === 'on' ? styles.statusOn : styles.statusOff]}
                onPress={() => toggleAppliance(item.id, item.status)}
              >
                <Text style={styles.statusText}>{item.status === 'on' ? 'ON' : 'OFF'}</Text>
              </TouchableOpacity>
            ...
            <View style={styles.filterSection}>
              ...
              <View style={styles.filterRow}>
                {[
                  ...
                  >
                    <Text style={[styles.filterChipText, filterStatus === item.key && styles.filterChipTextActive]}>{item.label}</Text>
                  </TouchableOpacity>
                ))}
              </View>
            </View>
            ...
            <View style={styles.filterSection}>
              ...
              <View style={styles.filterRow}>
                {[
                  ...
                  >
                    <Text style={[styles.filterChipText, sortBy === item.key && styles.filterChipTextActive]}>{item.label}</Text>
                  </TouchableOpacity>
                ))}
              </View>
            </View>
            ...
            <ScrollView style={styles.modalContent}>
              <View style={styles.modalForm}>
                ...
                <View style={styles.inputGroup}>
                  ...
                  <View style={styles.typeSelector}>
                    {[
                      ...
                      <TouchableOpacity
                        key={type.key}
                        style={[styles.typeOption, newAppliance.type === type.key && styles.typeOptionActive]}
                      ...
                      >
                        ...
                        />
                        <Text style={[styles.typeOptionText, newAppliance.type === type.key && styles.typeOptionTextActive]}>
                          {type.label}
                        </Text>
                      </TouchableOpacity>
                    ))}
                ...
                <View style={styles.inputGroup}>
                  ...
                  />
                  <Text style={styles.inputHint}>Power consumption when active</Text>
                </View>
                ...
                <View style={styles.inputGroup}>
                  ...
                  <TextInput
                    ...
                    keyboardType="numeric"
                  />
                  <Text style={styles.inputHint}>How many hours per day this device typically runs</Text>
                </View>
              </View>
            </ScrollView>
          </SafeAreaView>
              ...
              <View style={styles.modalForm}>
                ...
                <View style={styles.inputGroup}>
                  ...
                  <View style={styles.typeSelector}>
                    {[
                      ...
                      <TouchableOpacity
                        key={type.key}
                        style={[styles.typeOption, editingAppliance?.type === type.key && styles.typeOptionActive]}
                      ...
                      >
                        ...
                        />
                        <Text style={[styles.typeOptionText, editingAppliance?.type === type.key && styles.typeOptionTextActive]}>
                          {type.label}
                        </Text>
                      </TouchableOpacity>
                    ))}
                ...
                <View style={styles.inputGroup}>
                  ...
                  <TextInput
                    ...
                    placeholder="8"
                    placeholderTextColor="#6b7280"
                    value={editingAppliance?.average_hours_per_day?.toString()}
                    onChangeText={(text) => setEditingAppliance(prev => ({
                      ...prev,
                      average_hours_per_day: text.replace(/[^0-9]/g, '')
                    }))}
      ...
      >
        <View style={styles.controlsHeader}>
          <View style={styles.headerLeft}>
            ...
            <View style={styles.deviceStatsContainer}>
              <View style={styles.deviceStat}>
                <Text style={[styles.deviceStatNumber, styles.activeDeviceStat]}>{appliances.length}</Text>
          ...
          <View style={styles.headerRight}>
            <View style={styles.controlButtons}>
              ...
              >
                <MaterialIcons name="eco" size={16} color={energySavingMode ? "#ffffff" : "#a1a1aa"} />
                <Text style={[styles.controlButtonText, energySavingMode && styles.controlButtonTextActive]}>
              ...
              >
                <MaterialIcons name={viewMode === 'grid' ? 'view-list' : 'grid-view'} size={16} color="#a1a1aa" />
                <Text style={styles.controlButtonText}>{viewMode === 'grid' ? 'List' : 'Grid'}</Text>
              </TouchableOpacity>
            </View>
            <TouchableOpacity
        ...
        <View style={styles.statsContainer}>
          <View style={styles.statCard}>
            <MaterialIcons name="bolt" size={24} color="#10b981" style={styles.statIcon} />
            <Text style={styles.statValue}>{stats.totalUsage}W</Text>
            <Text style={styles.statLabel}>Current Usage</Text>
          </View>
          <View style={styles.statCard}>
            <MaterialIcons name="attach-money" size={24} color="#10b981" style={styles.statIcon} />
            <Text style={styles.statValue}>R{stats.monthlyCost}</Text>
            <Text style={styles.statLabel}>Monthly Cost</Text>
          </View>
          <View style={styles.statCard}>
            <MaterialIcons name="power" size={24} color="#10b981" style={styles.statIcon} />
            <Text style={styles.statValue}>{stats.activeDevices}</Text>
            <Text style={styles.statLabel}>Active</Text>
          </View>
          <View style={styles.statCard}>
            <MaterialIcons name="trending-up" size={24} color="#10b981" style={styles.statIcon} />
            <Text style={[
              styles.statValue,
              stats.efficiency === 'Excellent' && styles.efficiencyExcellent,
              stats.efficiency === 'Good' && styles.efficiencyGood,
              stats.efficiency === 'Fair' && styles.efficiencyFair,
              stats.efficiency === 'Poor' && styles.efficiencyPoor
            ]}>{stats.efficiency}</Text>
        ...
        <View style={styles.filterSection}>
          <View style={styles.searchContainer}>
            ...
            >
              <MaterialIcons name="filter-list" size={24} color="#a1a1aa" />
            </TouchableOpacity>
          </View>
            ...
            <ScrollView horizontal showsHorizontalScrollIndicator={false}>
              {appliances.filter(app => favorites.includes(app.id)).map(item => (
                <TouchableOpacity
                  ...
                  onPress={() => toggleAppliance(item.id, item.status)}
                >
                  <MaterialIcons name={getApplianceIcon(item.type)} size={24} color="#10b981" />
                  <Text style={styles.favoriteName}>{item.name}</Text>
                  <View style={[styles.favoriteStatus, item.status === 'on' ? styles.favoriteStatusOn : styles.favoriteStatusOff]} />
                </TouchableOpacity>
              ))}
            </ScrollView>
          </View>
        ...
        {filteredAppliances.length === 0 ? (
          <View style={styles.emptyState}>
            <MaterialIcons name="devices" size={64} color="#374151" />
            <Text style={styles.emptyTitle}>{searchQuery ? 'No devices found' : 'No Devices'}</Text>
